...
class FiscalApiClient
{
    ...
    /**
     * Download the immutable PDF generated by apiArca for an authorized document.
     */
    public function documentPdf(string|int $documentId): Response
    {
        try {
            $response = $this->binaryRequest()->get('/fiscal/documents/'.$this->pathSegment($documentId).'/pdf');
        ...
        if (! $response->successful()) {
            ...
            $message = trim((string) data_get(
                $payload,
                'error.message',
                data_get($payload, 'message', 'La API fiscal no pudo entregar el PDF del comprobante.')
            ));

            throw new FiscalApiException($message !== '' ? $message : 'La API fiscal no pudo entregar el PDF del comprobante.');
        }

        $contentType = strtolower((string) $response->header('Content-Type'));

        if (! str_starts_with($contentType, 'application/pdf')) {
            throw new FiscalApiException('La API fiscal devolvió una respuesta inválida al solicitar el PDF del comprobante.');
        }
    ...
    private function binaryRequest(?int $timeout = null): PendingRequest
    {
        return Http::baseUrl($this->baseUrl())
