    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h2 class="text-2xl font-bold text-slate-900">Ventas</h2>
                    <p class="mt-1 text-sm text-slate-500">Historial de ventas del comercio.</p>
                </div>
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody v-if="sales.data.length" class="divide-y divide-slate-100">
                        <tr v-for="sale in sales.data" :key="sale.id">
                            <td class="px-3 py-2 font-semibold text-slate-900">{{ sale.sale_number || `VENTA-${sale.id}` }}</td>
                            <td class="px-3 py-2">{{ sale.sold_at || '-' }}</td>
                            <td class="px-3 py-2">{{ sale.items_count }}</td>
                            <td class="px-3 py-2">{{ money(sale.total) }}</td>
                            <td class="px-3 py-2 text-right">
                                <Link :href="route('sales.show', sale.id)" class="rounded-lg border border-slate-300 px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50">Ver</Link>
                            </td>
                        </tr>
                    </tbody>
                    <tbody v-else>
