...
<template>
    ...
    <AuthenticatedLayout>
            ...
            <AppPanel title="Caja cerrada" subtitle="Abrí la caja antes de registrar efectivo. Solo puede haber una caja física abierta por sucursal.">
                <form class="grid gap-4" @submit.prevent="open">
                    <div class="app-field">
                        ...
                        <input v-model.number="openingForm.opening_amount" type="number" inputmode="decimal" min="0" step="0.01" autofocus class="w-full rounded-xl text-sm" placeholder="0,00" />
                    ...
                    <button type="submit" class="w-full rounded-xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white hover:bg-indigo-500 disabled:opacity-50 sm:w-auto" :disabled="openingForm.processing">Abrir caja</button>
                </form>
            </AppPanel>
        </div>
                ...
                <div class="grid gap-6">
                    ...
                    <AppPanel title="Movimientos" subtitle="Las ventas en efectivo se agregan automáticamente. Los registros son inmutables; las correcciones se hacen con un contramovimiento.">
                        <div class="grid gap-3 md:hidden">
                            <article v-for="movement in current.movements" :key="movement.id" class="rounded-xl border border-cyan-100/15 bg-slate-950/30 p-4">
                                <div class="flex items-start justify-between gap-3">
                                    <div class="min-w-0"><p class="font-semibold text-slate-100">{{ movementLabel(movement.type) }}</p><p class="mt-1 text-xs text-slate-400">{{ formatDate(movement.occurred_at) }} · {{ movement.created_by || 'Sistema' }}</p></div>
                                    <p class="shrink-0 text-lg font-bold" :class="movement.amount >= 0 ? 'text-emerald-200' : 'text-rose-200'">{{ movement.amount >= 0 ? '+' : '' }}{{ money(movement.amount) }}</p>
                                </div>
                                <p class="mt-3 break-words text-sm text-slate-300">{{ movement.description || 'Sin descripción.' }}</p>
                            </article>
                            <p v-if="current.movements.length === 0" class="rounded-xl border border-dashed border-cyan-100/20 p-4 text-center text-sm text-slate-400">Todavía no hay movimientos.</p>
                        </div>
                ...
                <div class="grid content-start gap-6">
                    <AppPanel title="Ingreso o egreso" subtitle="No uses esta acción para ventas; se incorporan desde el cobro." >
                        <form class="grid gap-4" @submit.prevent="addMovement">
                            ...
                            <div class="app-field"><label class="app-field-label">Importe</label><input v-model.number="movementForm.amount" type="number" inputmode="decimal" min="0.01" step="0.01" class="w-full rounded-xl text-sm" /><p v-if="movementForm.errors.amount" class="mt-1 text-xs text-rose-300">{{ movementForm.errors.amount }}</p></div>
                            ...
                            <button type="submit" class="w-full rounded-xl border border-cyan-200/30 px-4 py-2.5 text-sm font-semibold text-cyan-50 hover:bg-cyan-300/10 disabled:opacity-50 sm:w-auto" :disabled="movementForm.processing">Registrar movimiento</button>
                        </form>
                    </AppPanel>
                    <AppPanel title="Cerrar caja" subtitle="El esperado se guarda como foto del momento del cierre.">
                        <form class="grid gap-4" @submit.prevent="close">
                            ...
                            <div class="app-field"><label class="app-field-label">Dinero contado</label><input v-model.number="closingForm.counted_amount" type="number" inputmode="decimal" min="0" step="0.01" class="w-full rounded-xl text-sm" /><p v-if="closingForm.errors.counted_amount" class="mt-1 text-xs text-rose-300">{{ closingForm.errors.counted_amount }}</p></div>
                            ...
                            <button type="submit" class="w-full rounded-xl bg-rose-600 px-4 py-3 text-sm font-semibold text-white hover:bg-rose-500 disabled:opacity-50 sm:w-auto" :disabled="closingForm.processing">Confirmar cierre</button>
                        </form>
                    </AppPanel>
                </div>
            </div>
        ...
        <AppPanel class="mt-6" title="Historial de cajas" subtitle="Solo se muestran sesiones de la sucursal activa.">
            <div class="grid gap-3 md:hidden"><article v-for="session in history.data" :key="session.id" class="rounded-xl border border-cyan-100/15 bg-slate-950/30 p-4"><div class="flex items-start justify-between gap-3"><div><p class="font-semibold text-slate-100">{{ branch.name }}</p><p class="mt-1 text-xs text-slate-400">{{ session.opened_by || 'Sistema' }} · {{ formatDate(session.opened_at) }}</p></div><span class="rounded-full px-2 py-1 text-xs" :class="session.status === 'open' ? 'bg-emerald-400/15 text-emerald-100' : 'bg-slate-700 text-slate-200'">{{ session.status === 'open' ? 'Abierta' : 'Cerrada' }}</span></div><dl class="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt class="text-xs text-slate-400">Inicial</dt><dd class="font-semibold">{{ money(session.opening_amount) }}</dd></div><div><dt class="text-xs text-slate-400">Esperado</dt><dd class="font-semibold">{{ session.expected_amount_at_close === null ? '—' : money(session.expected_amount_at_close) }}</dd></div><div><dt class="text-xs text-slate-400">Contado</dt><dd class="font-semibold">{{ session.counted_amount === null ? '—' : money(session.counted_amount) }}</dd></div><div><dt class="text-xs text-slate-400">Diferencia</dt><dd class="font-semibold">{{ session.difference_amount === null ? '—' : money(session.difference_amount) }}</dd></div></dl><p class="mt-3 text-xs text-slate-400">Cierre: {{ formatDate(session.closed_at) }}</p><Link :href="route('cash-register.sessions.show', session.id)" class="mt-4 inline-flex min-h-11 w-full items-center justify-center rounded-lg border border-cyan-100/25 px-3 text-sm font-semibold text-cyan-100">Ver detalle</Link></article><p v-if="history.data.length === 0" class="rounded-xl border border-dashed border-cyan-100/20 p-4 text-center text-sm text-slate-400">Todavía no hay sesiones.</p></div>
