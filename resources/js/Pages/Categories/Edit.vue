...
const categoryWarnings = computed(() => {
    ...
    if (!form.is_active) warnings.push('La categoria quedara inactiva para el catalogo.');
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <form class="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_20rem]" @submit.prevent="submit">
            <AppPanel title="Datos de la categoria" subtitle="Mantiene coherente la estructura del catalogo y los filtros del negocio.">
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <div class="app-field">
                        ...
                        <input v-model="form.slug" type="text" class="w-full rounded-xl text-sm" />
                    </div>
                    <div class="app-field md:col-span-2">
                        <label class="app-field-label">Descripcion</label>
                        <textarea v-model="form.description" rows="3" class="w-full rounded-xl text-sm" />
                    </div>
                </div>
