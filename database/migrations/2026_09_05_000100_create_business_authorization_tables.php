...
return new class extends Migration
{
    public function up(): void
    {
        ...
        foreach (config('authorization.permissions', []) as $code => $module) {
            DB::table('permissions')->insert(['code' => $code, 'module' => $module, 'created_at' => $now, 'updated_at' => $now]);
        }
        DB::table('businesses')->orderBy('id')->each(function (object $business) use ($now): void {
            ...
            foreach (config('authorization.roles', []) as $code => $template) {
                $roleId = DB::table('roles')->insertGetId(['business_id' => $business->id, 'code' => $code, 'name' => $template['name'], 'is_system' => true, 'created_at' => $now, 'updated_at' => $now]);
            ...
            $staffRoleId = DB::table('roles')->insertGetId(['business_id' => $business->id, 'code' => 'legacy_staff', 'name' => 'Staff legado', 'is_system' => true, 'created_at' => $now, 'updated_at' => $now]);
            ...
            foreach ($staffPermissions as $permissionId) {
                DB::table('permission_role')->insert(['permission_id' => $permissionId, 'role_id' => $staffRoleId, 'created_at' => $now, 'updated_at' => $now]);
            }
            ...
            DB::table('users')->where('business_id', $business->id)->whereIn('role', ['admin', 'staff'])->orderBy('id')->each(function (object $user) use ($business, $now, $administratorRoleId, $staffRoleId, $branchIds): void {
                DB::table('role_user')->insert(['user_id' => $user->id, 'role_id' => $user->role === 'admin' ? $administratorRoleId : $staffRoleId, 'business_id' => $business->id, 'created_at' => $now, 'updated_at' => $now]);
                foreach ($branchIds as $branchId) {
                    DB::table('branch_user')->insert(['user_id' => $user->id, 'branch_id' => $branchId, 'business_id' => $business->id, 'created_at' => $now, 'updated_at' => $now]);
                }
            });
        });
    }
